...
OUT = Path("/home/cem/colcon_ws/src/anomaly_detection/belgeler/"
           "UR10e_Anomali_Tespiti_Teknik_Rapor.docx")
...
p("Bildiri birleşik skoru yalnızca BestF1 ile raporlamakta, yani bir **çalışma eşiği "
  ...
  "doğrulama** pencerelerindeki birleşik skorun 97. persentilidir. Ölçülen değer "
  "θ_birleşik = 0,6436 olup, temiz doğrulamada beklenen %3 yanlış alarm oranını "
  "vermektedir.")
...
p("`ur10e_anomaly_detection` paketi, hesabın tamamını ROS'tan bağımsız bir çekirdekte "
...
table(
    ...
    [["Abone", "/joint_states", "sensor_msgs/JointState  (effort = Amper)"],
     ["Abone", "/force_torque_sensor_broadcaster/ft_data", "geometry_msgs/WrenchStamped"],
    ...
    caption="Tablo 7.3. Düğümün arayüzü. `~/detail` sırasıyla s_kal, s_ham, z_kal, "
            ...
            "uyarlanabilir isabet, kalıntı isabet, ham isabet değerlerini taşır.",
    widths=[2.2, 6.2, 6.0], num_from=2)
p("Önemli parametreler: `tf_prefix` (varsayılan `ur10e_`; `/joint_states` içindeki "
  "eklem adlarıyla eşleşmelidir), `wrench_topic`, `stride` (25), `fts_frame` (`tool`), "
  "`max_wrench_age` (20 ms — bayat wrench ile skor üretilmez), `adaptive` (açık), "
  "`adaptive_window` (600 karar = 30 s), `adaptive_k` (8), `adaptive_warmup` (200), "
  "`consecutive_for_alarm` (2).")
...
h(2, "10.1. Motor kayması neden düşük kalıyor")
...
h(2, "10.2. Mutlak başarım değerlerinin düşmesi")
...
h(2, "10.3. FMU'nun ölçülen sınırlarının etkisi")
...
p("Bildirinin “Gelecek çalışmalar” bölümünde işaret ettiği üç maddeden biri — "
  ...
  "tamamlanmıştır. Sıradaki adım, düğümün gerçek robot üzerinde çalıştırılması ve "
  "gerçek arıza kayıtlarıyla eşiklerin yeniden doğrulanmasıdır.")


# ═══════════════════════════════════════════════════════════════════
h(1, "12. Yeniden üretilebilirlik")

h(2, "12.1. Çevrimdışı hat")
...
h(2, "12.2. Bildirinin özgün hattı (erratum)")
code("bash reproduce_erratum.sh erratum")

h(2, "12.3. Birleşim denetimi")
...
h(2, "12.4. Çevrimiçi düğüm")
code("ros2 launch ur10e_anomaly_detection detector.launch.py\n"
     "\n"
     "# bildirinin saf davranışına dönmek için:\n"
     "ros2 launch ur10e_anomaly_detection detector.launch.py \\\n"
     "     adaptive:=false consecutive_for_alarm:=1")

h(2, "12.5. Dosya envanteri")
table(
    ["Dosya", "Satır", "İşlev"],
    [["`prepare_dataset.py`", "196", "koşulara ayırma, düzgün ızgaraya örnekleme"],
     ...
     ["`ur10e_anomaly_detection/features.py`", "154", "çevrimiçi öznitelik motoru"],
     ["`ur10e_anomaly_detection/detector.py`", "213", "ROS'tan bağımsız çekirdek"],
     ["`ur10e_anomaly_detection/detector_node.py`", "258", "ROS 2 düğümü"],
     ["`ur10e_anomaly_detection/replay_publisher.py`", "169", "500 Hz parquet yayıncısı"]],
    caption="Tablo 12.1. Yazılım envanteri. Çevrimdışı hat 3.176, ROS 2 paketi "
            "892 satırdır.",
