#!/usr/bin/env python3
"""
Red detection / mock perception - v5 (gerçek robot topic'leri)
...
Sim'den fark: varsayılan topic'ler gerçek hücreye göredir.
  image_topic      : /intensity   (SICK Visionary-T Mini intensity görüntüsü)
  pointcloud_topic : /sick_points (pointcloud_transformer_ur çıkışı, /points'ten)
UYARI: SICK Visionary-T Mini renkli (RGB) görüntü yayınlamaz; intensity mono'dur.
Renk tabanlı tespit için image_topic'i gerçek bir RGB kameranın topic'ine
ayarlayın (çözünürlüğü pointcloud ile aynı olmalı, çünkü piksel indeksi
doğrudan buluta uygulanır).
"""
...
MONO_ENCODINGS = ("mono8", "mono16", "16UC1", "8UC1")
...
class HarmonyRedDetectionV5(Node):
    def __init__(self):
        super().__init__("harmony_red_detection")

        self.declare_parameter("image_topic", "/intensity")
        self.declare_parameter("pointcloud_topic", "/sick_points")
        ...
        if CV_AVAILABLE:
            ...
            self.get_logger().warning("OpenCV not available; red detection disabled")
        ...
        self.get_logger().info(f"HarmonyRedDetectionV5 (REAL) initialized, world_frame={self.world_frame}")
        self.get_logger().info(f"  image_topic={self.image_topic}, pointcloud_topic={self.pointcloud_topic}")
    ...
    def _image_cb(self, msg: Image):
        if not CV_AVAILABLE:
            return
        if msg.encoding in MONO_ENCODINGS and not self._mono_warned:
            self._mono_warned = True
            self.get_logger().warning(
                f"'{self.image_topic}' encoding={msg.encoding} (tek kanal). Renk tabanlı "
                f"tespit çalışmaz; image_topic parametresini bir RGB kameraya ayarlayın."
            )
        try:
    ...
    def _detect_red(self, image_bgr):
        try:
            if image_bgr.ndim != 3:
                # Tek kanallı (mono/intensity) görüntüde renk bilgisi yok.
                return []
