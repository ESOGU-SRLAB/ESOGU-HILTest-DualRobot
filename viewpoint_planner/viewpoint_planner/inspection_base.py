...
The concrete node (inspection_executor_node) plans the arm (collision-aware, via the
real MoveIt) to each of its coverage viewpoints, dispatches the trajectory, then grabs
the latest SICK cloud, transforms it into the world frame, and writes per viewpoint:
    <subtree>/pcds/<N>.pcd   -- the cloud
    <subtree>/poses/<N>.txt  -- OctoMap origin "x y z qx qy qz qw" (world frame)

This base holds the machinery that makes a run REPEATABLE rather than merely possible:

  * a TRAJECTORY CACHE with a record-once / replay-forever contract, so the path the
    arm drives is the one that was validated, not a fresh sample from a randomized
    planner on every run;
  * 2*pi GOAL UNWINDING, so a goal stored as theta + 2*pi does not cost a full extra
    turn to reach an identical pose;
  * NEAREST-BRANCH IK, so a viewpoint is approached from the closest of its several
    valid arm configurations instead of an arbitrary one;
  * start handling (rebase-onto-current, else a collision-aware walk to the recorded
    start) so a cached path is never dispatched from a pose it was not recorded at;
  * the ground plane, collision padding and measured-arrival helpers.

ORIGIN AND DRIFT WARNING: this file is a sibling of
`multirobot_viewpoint_planner/inspection_base.py`, which drives the two-arm cell. The
two were the same file on 2026-09-09 and the machinery below is the version proven on
the real cell there. They are deliberately SEPARATE copies -- this package is the base
that multirobot depends on, so importing the other way round would be circular -- but
that means a fix made in one does not reach the other. When you change behaviour here,
check whether the multirobot copy needs the same change.
"""
import json
import os
import re
...
class InspectionNodeBase(Node):
    """Base for a single-arm inspection node. `robot_tag` ("ur") selects the plan JSON
    key (`<robot_tag>_viewpoints`) and the trajectory-cache filename prefix."""
    ...
    # ------------------------------------------------------------------ #
    def should_run(self):
        """True if this arm should run. There is only one arm in this package, so the
        default is always True; the multirobot copy resolves only_ur/only_kawasaki here."""
        return True
    ...
    def _arm_link_names(self, scene):
        """The links to which collision padding is applied. Overridden by the concrete
        node ('ur10e_*' here)."""
        raise NotImplementedError

    def _padding_for_link(self, name, base_padding):
        """Absolute padding for ONE link, in metres. Default: the same margin for every
        link. Overridden where some bodies need a bigger margin than others."""
        return base_padding
    ...
    def _apply_collision_padding(self):
        ...
        diff = PlanningScene()
        diff.is_diff = True
        # Padding is per LINK, not global, so a subclass can inflate some bodies more
        # than others (this package pads the inspected chassis harder than the arm --
        # see _padding_for_link). A link whose hook returns <= 0 is left unpadded.
        diff.link_padding = [
            LinkPadding(link_name=n, padding=self._padding_for_link(n, padding))
            for n in arm_links if self._padding_for_link(n, padding) > 0.0]
        ...
        if not self._wait_future(af, 5.0) or af.result() is None:
            ...
            return
        pads = {}
        for lp in diff.link_padding:
            pads[round(lp.padding, 3)] = pads.get(round(lp.padding, 3), 0) + 1
        detail = ", ".join(f"{n} link(s) at {v * 100:.1f} cm"
                           for v, n in sorted(pads.items(), reverse=True))
        self.get_logger().info(
            f"Collision padding applied: {detail}. success={af.result().success}.")
    ...
    # Planning.
    # ------------------------------------------------------------------ #
    def _plan(self, joint_positions, joint_names, label, attempts=None):
        """Plan to a joint goal, re-issuing the whole call up to `plan_attempts` times
        (OMPL is stochastic, so a fresh call can succeed where one failed). `attempts`
        overrides that count for callers that only want a single look."""
        moveit2 = self.moveit
        if attempts is None:
    ...
    def _plan_best_branch(self, candidates, joint_names, label):
        ...
        Two rules keep a hopeless branch from eating the run (added 2026-09-10, after a
        single-UR recording run spent 2 x 40 s per viewpoint on 692 / 722 deg branches
        that had no path, while the 165 deg one planned in a fraction of a second):

          * LOSSLESS CUT-OFF. A trajectory starts at the current pose and ends at the
            candidate goal, so per joint its summed |delta| is at least |goal - start|
            (triangle inequality), i.e. _traj_travel >= _goal_travel over the same
            revolute joints. Once a branch has planned with travel T, a candidate whose
            straight-line goal distance is already >= T cannot produce anything shorter.
            Candidates arrive sorted by that distance, so everything from there on is
            skipped -- without any loss in the path that gets recorded.
          * ONE LOOK AT ALTERNATIVES. Until SOMETHING has planned, each candidate gets
            the full `plan_attempts` retries (we still need a path). After that the
            rest are optional improvements and get a single planning call each.
        ...
        for cand_goal, goal_travel in candidates:
            if best_traj is not None and goal_travel >= best_travel:
                pruned = sum(1 for _, t in candidates if t >= goal_travel)
                break
            traj = self._plan(
                cand_goal, joint_names,
                f"{label} [branch {np.rad2deg(goal_travel):.0f} deg]",
                attempts=(1 if best_traj is not None else None))
            if traj is None:
                continue
            planned += 1
        ...
        if pruned:
            self.get_logger().info(
                f"[{label}] skipped {pruned} farther branch(es) without planning: their "
                f"goal alone is >= the {np.rad2deg(best_travel):.0f} deg path already found, "
                "so none of them could be shorter.")
    ...
    def run(self):
        ...
        if not self.should_run():
            self.get_logger().info(
                f"{self.arm_label} node idle (should_run() returned False); exiting.")
            return
